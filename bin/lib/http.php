...
class vhttpd_http
{
	...
	public function parse($requestHeader)
	{
		...
		// 解析URL得到 GET/COOKIE 全局变量
		$this->_parseGetCookie();
	...
	public function getResponse($result)
	{
		...
		if(isset($result['ext'])){
			$this->_setResponseType($result['ext']);
		}
		// 如果有设置文件扩展名则要自动设置对应的 Content-Type
		...
		$headerArr[] = 'Server: vhttpd '.VHTTPD_VERSION.' for Linux';
		$headerArr[] = 'Author: V哥';
	 ...
	 */
	private function _setResponseType($ext)
	{
		$conf = vhttpd_sys::getConf();
		$ext = strtolower($ext);
		$type = isset($conf['mime_types'][$ext]) ? $conf['mime_types'][$ext] : $conf['mime_types']['other'];
	 ...
	 *
	 */
	private function _parseGetCookie()
	{
		// 解析 GET
		...
		if(isset($this->_header['cookie'])){
			$_COOKIE = $this->_parseField($this->_header['cookie']);
		}
	}
